...
class Adapter(Adaptee):
    """
    The adapter_design_pattern makes the Adaptee's interface compatible with the Target's
    interface via multiple inheritance.
    """

    def request(self, a, b) -> str:
        """
        Here we have created an adapter_design_pattern (the request function) that looks exactly like our client interface, and
        it calls the specific_request function in a way that makes it compatible to our code base.
        In our code we can still pass (1,2) to the request function and adjust the contract.
        """
        return f"adapter_design_pattern: (TRANSLATED) {self.specific_request(a=a, c=b)}"
...
if __name__ == "__main__":
    ...
    # implement an adapter that first of all enables us to call the specific request method but in addition does not
    # require us to change our way of calling the request method.

    # This adapter_design_pattern has the interface of the Target class but calls the specific request method under the hood.
    adapter = Adapter()
